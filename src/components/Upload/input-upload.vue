...
/**
 * 通用上传控件，两种形态（trigger）：
 * - `input`（默认）：「只读输入框 + 输入框内嵌上传图标」形态 ——
 *   输入框展示 value（通常取文件地址末段作为文件名），过长截断并以 tooltip 展示完整名；
 *   右侧移除按钮可清空已选文件；v-model:value 保存上传返回的地址。
 * - `button`：只渲染一个主按钮（上传图标 + buttonText），不带只读输入框 / 清除按钮 ——
 *   适合「选文件 → 解析拿数据」场景（无地址语义，不回填 value，仅 success 回传 raw）。
 * - 两种形态共用同一套「选择文件 → 调 upload（或默认 /upload）→ loading → 事件」逻辑；
 *   传入 upload 可替换为专用接口（如导出模板解析上传、偏远数据 Excel 解析）。
 * - success 事件携带原始返回，供调用方取额外字段（如解析结果）。
 */

/** 自定义上传：取代默认 /upload，返回地址与原始结果（失败返回 null）；url 可缺省（解析型接口无地址） */
type UploadFn = (file: File, onProgress?: (percent: number) => void) => Promise<{ url?: string; raw?: unknown } | null>;

interface Props {
  /** 已上传文件地址（/upload 返回的 url），直接作为输入框展示值（仅 input 形态使用） */
  value?: string;
  ...
  upload?: UploadFn;
  /** 形态：input = 只读输入框 + 图标按钮（默认）；button = 单个主按钮 */
  trigger?: 'input' | 'button';
  /** button 形态的按钮文案（默认「选择文件」） */
  buttonText?: string;
}
...
const emit = defineEmits<{
  ...
  success: [raw: unknown];
  /** 移除已选文件（清空值，仅 input 形态会触发） */
  remove: [];
}>();

const uploading = ref(false);

/** 按钮形态（只渲染主按钮，不渲染只读输入框） */
const isButton = computed(() => props.trigger === 'button');

/** 按钮文案：缺省复用全局「选择文件」 */
const resolvedButtonText = computed(() => props.buttonText || $t('common.chooseFile'));
...
<template>
  <!--
    button 形态：只渲染一个主按钮。
    w-auto!：naive 给 .n-upload 写死了 width:100%，在 flex 行内会独占整行
    （把前后控件挤到上下两行），故覆盖为内容宽度
  -->
  <NUpload
    v-if="isButton"
    class="w-auto!"
    ...
    @change="handleChange"
  >
    <LButton type="primary" :disabled="disabled" :loading="uploading">
      <template #icon>
  ...
  <NTooltip v-else :disabled="!displayName" placement="top" trigger="hover">
    <template #trigger>
