...
import { fetchCreateCustomsType, fetchUpdateCustomsType } from '@/service/api/data-manage-archive';
...
const formItems: FormItemConfig[] = [
  ...
  { key: 'inner', label: $t(`${F}.inner`), type: 'select', span: 12, options: yesNoOptions.value },
  { key: 'note', label: noteLabel, type: 'textarea', span: 24 }
];

const createDefault = (): Record<string, unknown> => ({ name: '', inner: 0, note: '', status: 1 });
...
async function handleSubmit() {
  ...
  try {
    if (drawerMode.value === 'create') {
      await fetchCreateCustomsType(model as Partial<Row>);
      window.$message?.success($t('common.createSuccess'));
    } else {
      await fetchUpdateCustomsType({ ...(model as object), _id: currentId.value } as Partial<Row>);
