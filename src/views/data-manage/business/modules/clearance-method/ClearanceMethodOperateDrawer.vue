...
import { fetchCreateCustomsClear, fetchUpdateCustomsClear } from '@/service/api/data-manage-archive';
...
const F = 'page.dataManage.business.clearanceMethod';
...
async function handleSubmit() {
  ...
  try {
    if (drawerMode.value === 'create') {
      await fetchCreateCustomsClear(model as Partial<Row>);
      window.$message?.success($t('common.createSuccess'));
    } else {
      await fetchUpdateCustomsClear({ ...(model as object), _id: currentId.value } as Partial<Row>);
