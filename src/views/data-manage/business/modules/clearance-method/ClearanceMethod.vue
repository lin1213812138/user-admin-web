...
import { fetchDeleteCustomsClear, fetchGetCustomsClearList } from '@/service/api/data-manage-archive';
import { Table, TableColumnConfig, useVxeTable } from '@/components/Table';
import type { VxeColumnConfig } from '@/components/Table';
import type { FormItemConfig } from '@/components/Form/index.vue';
import ClearanceMethodOperateDrawer from './ClearanceMethodOperateDrawer.vue';
import { useArchiveStatusOptions } from '@/views/data-manage/components/shared';

type Row = Api.DataManage.BusinessClearanceMethod;
...
>({
  api: async ({ current, size }) => {
    ...
    const { data: res, error } = await fetchGetCustomsClearList(
  ...
  cacheKey: 'data-manage-business-clearance-method'
});
...
async function handleDelete(ids: string[]) {
  await fetchDeleteCustomsClear(ids);
  window.$message?.success($t('common.deleteSuccess'));
  checkedRows.value = [];
  getData();
}

const drawerRef = ref<InstanceType<typeof ClearanceMethodOperateDrawer> | null>(null);
...
<template>
  <div class="h-full w-full flex flex-col gap-12px">
    ...
    <ClearanceMethodOperateDrawer ref="drawerRef" @submitted="getData" />
  </div>
</template>
