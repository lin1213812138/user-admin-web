...
import { fetchCreateSalesTerms, fetchUpdateSalesTerms } from '@/service/api/data-manage-archive';
...
const F = 'page.dataManage.business.salesTerms';
...
async function handleSubmit() {
  ...
  try {
    if (drawerMode.value === 'create') {
      await fetchCreateSalesTerms(model as Partial<Row>);
      window.$message?.success($t('common.createSuccess'));
    } else {
      await fetchUpdateSalesTerms({ ...(model as object), _id: currentId.value } as Partial<Row>);
