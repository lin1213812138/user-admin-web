...
import { fetchDeleteExportReason, fetchGetExportReasonList } from '@/service/api/data-manage-archive';
import { Table, TableColumnConfig, useVxeTable } from '@/components/Table';
import type { VxeColumnConfig } from '@/components/Table';
import type { FormItemConfig } from '@/components/Form/index.vue';
import ExportReasonOperateDrawer from './ExportReasonOperateDrawer.vue';
...
>({
  api: async ({ current, size }) => {
    ...
    const { data: res, error } = await fetchGetExportReasonList(
...
async function handleDelete(ids: string[]) {
  await fetchDeleteExportReason(ids);
  window.$message?.success($t('common.deleteSuccess'));
  checkedRows.value = [];
  getData();
}

const drawerRef = ref<InstanceType<typeof ExportReasonOperateDrawer> | null>(null);
...
<template>
  <div class="h-full w-full flex flex-col gap-12px">
    ...
    <ExportReasonOperateDrawer ref="drawerRef" @submitted="getData" />
  </div>
</template>
