...
import { fetchCreateProblemGroup, fetchUpdateProblemGroup } from '@/service/api/data-manage-archive';
...
async function handleSubmit() {
  ...
  try {
    if (drawerMode.value === 'create') {
      await fetchCreateProblemGroup(model as Partial<Row>);
      window.$message?.success($t('common.createSuccess'));
    } else {
      await fetchUpdateProblemGroup({ ...(model as object), _id: currentId.value } as Partial<Row>);
