...
const { data, loading, columnConfigs, columns, pagination, getData, persistColumns, resetColumns } = useVxeTable<
  Api.ChannelQuote.ChannelQuoteList<Api.ChannelQuote.ShipChannel>,
  Api.ChannelQuote.ShipChannel
>({
  api: ({ current, size }) => {
    ...
    return fetchGetChannelQuoteList<Api.ChannelQuote.ShipChannel>(
      ARCHIVE,
  ...
  columns: () =>
    [
      { key: 'code', title: $t('page.channelQuote.ship.code'), type: 'detail', visible: true, sortable: false },
      { key: 'name', title: $t('page.channelQuote.ship.name'), visible: true, sortable: false },
      {
        key: 'status',
...
const formItems = computed<FormItemConfig[]>(() => [
  {
    key: 'code',
    label: $t('page.channelQuote.ship.code'),
    type: 'input',
    required: true,
    span: 12,
    placeholder: $t('page.channelQuote.ship.form.codePlaceholder')
  },
  {
    key: 'name',
    label: $t('page.channelQuote.ship.name'),
    type: 'input',
    required: true,
    span: 12,
    placeholder: $t('page.channelQuote.ship.form.namePlaceholder')
  },
  {
    key: 'status',
...
const checkedRows = ref<Api.ChannelQuote.ShipChannel[]>([]);

function handleSelectionChange(records: Api.ChannelQuote.ShipChannel[]) {
...
const drawerTitle = computed(() => {
  const base = $t('page.channelQuote.ship.title');
  const op =
...
function openDrawer(mode: 'create' | 'edit' | 'detail', row?: Api.ChannelQuote.ShipChannel) {
...
async function handleSubmit() {
  ...
  try {
    if (drawerMode.value === 'create') {
      await fetchCreateChannelQuote<Api.ChannelQuote.ShipChannel>(
        ARCHIVE,
        model as unknown as Partial<Api.ChannelQuote.ShipChannel>
      );
      window.$message?.success($t('common.createSuccess'));
    } else {
      await fetchUpdateChannelQuote<Api.ChannelQuote.ShipChannel>(
        ARCHIVE,
        model as unknown as Api.ChannelQuote.ShipChannel
      );
